...
## As there is no other function that depicts the same plot as our function, and the variables necessary for those plots (namely residuals, fitted values, etc.) have been verified, we don't use either all.equal() or bench::mark().
```
...
## As this is a customized function, and there is no function that we can compare with, we don't use either all.equal() or bench::mark().
```
